...
from .views import (
    DashboardView, UserListView, UserCreateView, AdminSetupView, AdminLoginView,
...
urlpatterns = [
    # ✅ PWAのstart_url対策：ルートアクセスは user-login にリダイレクト
    path('', lambda request: redirect('/user-login/'), name='root_redirect'),
    ...
    # ✅ PWA用のルーティング（manifest.json や serviceworker.js がこれで提供される）
